...
const handler = async (req, res) => {
    ...
    if (session) { 
       if (req.method === "PUT") {
            ...
            const newL = await client.db("Panathon").collection("Users").findOne({"uid": userid});           
            ...
            if (!newL.project.equals(project._id)) {
                return res.status(401).send({message: "User cannot be set to leader as it is not in the project!"})
            }

            const options = { upsert: false };
            const filter = {_id: project._id};
            const updateDoc = { $set: { leader: newL.uid}};

            const projectResult = await projectCollection.updateOne(filter, updateDoc, options)
            console.log(`A document was updated with the _id: ${projectResult.insertedId}`);
        
            return res.status(204).send({message: projectResult})
